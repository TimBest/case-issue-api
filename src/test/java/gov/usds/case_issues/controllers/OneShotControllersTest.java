...
public class OneShotControllersTest extends ControllerTestBase {
	...
	private ResultActions getUser(String... origin) throws Exception {
		return doGet(UserDetailsApiController.USER_INFO_ENDPOINT, origin);
	}

	private ResultActions getCsrf(String... origin) throws Exception {
		return doGet("/csrf", origin);
	}

	@Test
	@WithAnonymousUser
	public void getUser_anonymous_forbidden() throws Exception {
		getUser().andExpect(status().isForbidden());
	}
	...
	public void getCsrf_badOrigin_forbidden() throws Exception {
		getCsrf(ORIGIN_NOT_OK).andExpect(status().isForbidden());
	}
}
